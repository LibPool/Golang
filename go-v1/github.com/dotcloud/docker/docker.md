# github.com/dotcloud/docker

> 标签: com, docker, dotcloud, github.com, go, golang

## 简介

Go module，由 index.golang.org 收录：github.com/dotcloud/docker。

Go 语言目前为大版本 1.x；本库收录于 go-v1。

## 官网

- Go 文档：https://pkg.go.dev/github.com/dotcloud/docker
- 源码仓库：https://github.com/dotcloud/docker

## 历史版本号

- 当前版本：v28.5.2+incompatible

- v28.1.0-rc.2+incompatible
- v28.1.1+incompatible
- v28.2.0+incompatible
- v28.2.0-rc.1+incompatible
- v28.2.0-rc.2+incompatible
- v28.2.1+incompatible
- v28.2.2+incompatible
- v28.3.0+incompatible
- v28.3.0-rc.1+incompatible
- v28.3.0-rc.2+incompatible
- v28.3.1+incompatible
- v28.3.2+incompatible
- v28.3.3+incompatible
- v28.4.0+incompatible
- v28.4.0-rc.1+incompatible
- v28.4.0-rc.2+incompatible
- v28.5.0+incompatible
- v28.5.0-rc.1+incompatible
- v28.5.1+incompatible
- v28.5.2+incompatible
- 共 40 个版本，完整清单见 Go module proxy。

## 获取地址

- go mod 下载：`go get github.com/dotcloud/docker@v28.5.2+incompatible`
- 模块代理：https://proxy.golang.org/github.com/dotcloud/docker/@v/list
